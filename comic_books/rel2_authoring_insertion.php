	...
	<body>
		<?php
			...
			**********************************/
			...
			# FORM START
			echo "<form method = 'post' action = 'rel3_starring_insertion.php'>";
			...
			**********************************/
			...
			if($resultWriter = mysqli_query($conn, $sqlWriter)) {
				...
				echo "<h3>ERROR. Cannot execute $sqlWriter: </h3>" . mysqli_error($conn);
			}
			...
			if($resultArtist = mysqli_query($conn, $sqlArtist)) {
				...
				echo "<h3>ERROR. Cannot execute $sqlArtist: </h3>" . mysqli_error($conn);
			}
